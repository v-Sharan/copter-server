...
class UAVBase(UAV):
    ...
    @property
    def gimbal(self) -> Gimbal:
        """Returns the Gimbal Object that represents the corresponding to the UAV.

        Returns:
            the Gimbal Object that represents the corresponding to the UAV
        """
        return self._gimabl
...
class UAVDriver(Generic[TUAV], metaclass=ABCMeta):
    ...
    def engine_start(
        ...
        """Asks the driver to send a signal to the given UAVs to engine start signal.
        Each of the UAVs are assumed to be managed by this driver.

        Typically, you don't need to override this method when implementing
        a driver; override ``_engine_start_sginal()`` and
        optionally ``_engine_start_sginal_broadcast()`` instead.
        ...
        return self._dispatch_request(
            uavs,
            "wakeup request",
            self._engine_start_signal,
            getattr(self, "_engine_start_signal", None),
            transport=transport,
        )
    ...
    def send_mission_download_signal(self, uavs: list[TUAV]):
        """Asks the driver to send a signal to the given UAVs that makes them
        fly to a given target coordinate. Every UAV passed as an argument is
        assumed to be managed by this driver.

        Typically, you don't need to override this method when implementing
        a driver; override ``_send_mission_download_signal_single()`` instead.
        ...
        return self._dispatch_request(
            uavs,
            "Mission Download signal",
            self._send_mission_download_signal_single,
        )
    ...
    def send_loiter_mode(
        ...
        Typically, you don't need to override this method when implementing
        a driver; override ``_send_loiter_mode_single()`` and
        ...
        return self._dispatch_request(
            uavs,
            "qloiter mode signal",
            self._send_loiter_mode_single,
            getattr(self, "_send_guided_mode_broadcast", None),
            transport=transport,
        )

    def send_guided_mode(
        self, uavs: list[TUAV], transport: Optional[TransportOptions] = None
    ):
        """Asks the driver to send a return-to-home signal to the given
        UAVs, each of which are assumed to be managed by this driver.
        ...
        Typically, you don't need to override this method when implementing
        a driver; override ``_send_guided_mode()`` and
        optionally ``_send_guided_mode_broadcast()`` instead.
    ...
    def _send_speed_correction(self, uav: TUAV, *, speed_ms: int) -> None:
        """Asks the driver to request a single UAV to switch to low-power mode.
    ...
    def _send_mission_download_signal_single(
        self, uav: TUAV, target: GPSCoordinate
    ) -> None:
        """Asks the driver to send a "fly to target" signal to a single UAV
        managed by this driver.
