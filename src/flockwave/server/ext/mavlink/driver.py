...
class MAVLinkDriver(UAVDriver["MAVLinkUAV"]):
    ...
    async def _skip_waypoint(self, uav: "MAVLinkUAV", skip_point: int) -> bool:
        # Effectively the same as shutdown, but without the attempt to stop the
        # motors (so drones where the motors are running will not be affected)
        message = spec.mission_set_current(seq=skip_point)
        await self.send_packet(message, target=uav)
        return True
    ...
    async def _send_loiter_mode_single(
        self, uav: "MAVLinkUAV", *, transport=None
    ) -> None:
        channel = transport_options_to_channel(transport)
        await uav.set_mode(19, channel=channel)
    ...
    async def _send_auto_mode_single(
        ...
        channel = transport_options_to_channel(transport)
        await uav.set_mode("auto", channel=channel)
    ...
    async def _fly_to_in_guided_mode(self, target: GPSCoordinate) -> None:
        ...
        try:
            await self.driver.send_packet_with_retries(
                message, self, wait_for_response=response, timeout=0.2, retries=1
            )
        except TooSlowError:
